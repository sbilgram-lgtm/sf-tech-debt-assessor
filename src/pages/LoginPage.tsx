...
const CATEGORY_CHECKS: Record<string, CheckItem[]> = {
  ...
  'Code Quality': [
    ...
    { title: 'Hardwired IVs or Keys in Crypto Calls (PMD: ApexBadCrypto)', severity: 'critical' },
    { title: 'DML / SOQL Without CRUD Permission Checks (PMD: ApexCRUDViolation)', severity: 'critical' },
    { title: 'Database.executeAnonymous() Usage — Arbitrary Code Execution (PMD: ApexDangerousMethods)', severity: 'critical' },
    { title: 'PageReference from User-Controlled Input — Open Redirect (PMD: ApexOpenRedirect)', severity: 'critical' },
    { title: 'Hardcoded Authorization Headers — Use Named Credentials (PMD: ApexSuggestUsingNamedCred)', severity: 'high' },
    { title: 'URL Parameters Used Without Escaping — XSS Risk (PMD: ApexXSSFromURLParam)', severity: 'high' },
    { title: 'High Cyclomatic Complexity — Too Many Decision Branches (PMD: CyclomaticComplexity)', severity: 'high' },
    { title: '@AuraEnabled Properties with Inaccessible Getters (PMD: InaccessibleAuraEnabledGetter)', severity: 'high' },
    { title: 'equals() Without hashCode() or Vice Versa — Broken Map/Set Contract (PMD: OverrideBothEqualsAndHashcode)', severity: 'high' },
    { title: 'Class Names Shadow Built-In Apex Types (PMD: TypeShadowsBuiltInNamespace)', severity: 'high' },
    { title: 'SOQL LIMIT 1 Result Without Null Check — NullPointerException Risk (Graph Engine)', severity: 'high' },
    { title: 'SOQL Bind Variables Without Null Checks — Full Table Scan Risk (Graph Engine)', severity: 'high' },
    { title: 'Multiple Mass Schema Lookups in Same Class (Graph Engine: AvoidMultipleMassSchemaLookups)', severity: 'high' },
  ...
  'Sharing & Security': [
    ...
    { title: 'Objects with External Public Read/Write OWD', severity: 'critical' },
    { title: 'Objects with Internal Public Read Only OWD', severity: 'medium' },
    { title: 'Objects with External Public Read Only OWD', severity: 'medium' },
    ...
    { title: 'Standard Profiles With No Active Users Assigned', severity: 'low' },
  ...
  'Test Coverage': [
    ...
    { title: 'Test Classes That Never Call System.runAs() (PMD: ApexUnitTestClassShouldHaveRunAs)', severity: 'medium' },
    { title: 'Assert Statements Without a Message Parameter (PMD: ApexAssertionsShouldIncludeMessage)', severity: 'low' },
    { title: 'Test Methods Using Deprecated testMethod Keyword (PMD: ApexUnitTestMethodShouldHaveIsTestAnnotation)', severity: 'low' },
  ],
  'Org Limits': [
...
const CATEGORIES = [
  // ── Security & Access ──────────────────────────────────────────
  { icon: '🔒',  name: 'Sharing & Security',      checks: 29, group: 'Security & Access' },
  ...
  { icon: '🔀',  name: 'Flow Quality',            checks: 6,  group: 'Code & Development' },
  // ── Performance & Limits ───────────────────────────────────────
